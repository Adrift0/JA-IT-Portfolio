...
const headshot = "/jason-headshot_bc28959f.png";
const cvDownload = "/manus-storage/JasonAlebord-ITCV_dda695e3.docx";
const linkedInUrl = "https://www.linkedin.com/jasonalebord";
...
function ToneIcon({ icon: Icon, tone }: { icon: LucideIcon; tone: string }) {
  ...
  return (
    <div className={`icon-wrapper grid size-11 place-items-center rounded-2xl ring-1 ${styles[tone] ?? styles.blue}`}>
...
function MetricCard({ value, label, index }: { value: string; label: string; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`metric-card scroll-animate bg-[#0b1424]/90 p-6 text-center stagger-${index + 1} whitespace-nowrap`}
    >
      <p className="text-3xl font-black text-white md:text-4xl">{value}</p>
      <p className="mt-2 text-xs font-medium text-slate-400 leading-tight">{label}</p>
    </div>
  );
}
...
function CompetencyCard({ item, index }: { item: typeof competencies[0]; index: number }) {
  ...
  return (
    <article
      ref={ref}
      className={`competency-card scroll-animate rounded-[1.6rem] border border-white/10 bg-slate-800/70 p-6 stagger-${(index % 3) + 1}`}
    >
...
function ImpactCard({ item, index }: { item: typeof impacts[0]; index: number }) {
  const ref = useScrollAnimation();
  return (
    <article
      ref={ref}
      className={`impact-card scroll-animate rounded-[1.6rem] border border-white/10 border-l-4 ${item.border} bg-slate-800/70 p-6 stagger-${index + 1}`}
    >
      <ToneIcon
        icon={item.icon}
        tone={item.border.includes("teal") ? "teal" : item.border.includes("amber") ? "amber" : "blue"}
      />
      <h3 className="mt-5 text-xl font-bold text-white">{item.title}</h3>
      <p className="mt-3 text-sm leading-6 text-slate-400">{item.copy}</p>
    </article>
  ...
  return (
    <main className="min-h-screen bg-[#08101f] text-slate-100 selection:bg-electric/30 selection:text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#08101f]/78 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
          <a href="#top" className="flex items-center gap-3" aria-label="Jason Alebord profile home">
            ...
            <span className="hidden text-xs font-bold uppercase tracking-[0.22em] text-slate-300 sm:block">IT Professional Profile</span>
          </a>
      ...
      <section id="top" className="relative overflow-hidden px-5 py-16 sm:px-8 md:py-24 lg:px-10">
        <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]">
          ...
          <div className="order-1 text-center lg:order-2 lg:text-left">
            ...
            <h1 className="mt-5 text-5xl font-black leading-[0.95] tracking-[-0.055em] text-white sm:text-6xl lg:text-7xl">IT Solutions Engineer</h1>
              ...
              {certifications.map((cert, index) => {
                ...
                return (
                  <span
                    key={cert}
                    ref={ref}
                    className={`cert-badge scroll-animate rounded-xl border border-electric/45 bg-slate-800/80 px-4 py-2 text-xs font-black tracking-[0.16em] text-white shadow-[0_0_14px_rgba(59,130,246,0.15)] stagger-${(index % 7) + 1}`}
                  >
                    {cert}
                  </span>
                );
      ...
      <section className="border-y border-white/10 bg-[linear-gradient(135deg,rgba(20,184,166,0.08),rgba(59,130,246,0.08))] px-5 py-10 sm:px-8 lg:px-10">
        <div className="mx-auto grid max-w-7xl gap-px overflow-hidden rounded-3xl border border-white/10 bg-white/10 md:grid-cols-5">
          {impactStats.map(([value, label], index) => (
            <MetricCard key={label} value={value} label={label} index={index} />
          ))}
        </div>
      </section>
            ...
            {compliance.map((item, index) => {
              ...
              return (
                <span
                  key={item}
                  ref={ref}
                  className={`compliance-badge scroll-animate rounded-xl border border-amber/45 bg-slate-800/80 px-5 py-2.5 text-sm font-black tracking-[0.12em] text-amber shadow-[0_0_14px_rgba(245,158,11,0.12)] stagger-${(index % 5) + 1}`}
                >
                  {item}
                </span>
              );
      ...
      <footer className="border-t border-white/10 px-5 py-8 text-center sm:px-8 lg:px-10">
        <div className="mx-auto max-w-6xl">
          <p className="text-sm text-slate-500">Jason Alebord · St. Petersburg, FL · IT Solutions Engineer</p>
